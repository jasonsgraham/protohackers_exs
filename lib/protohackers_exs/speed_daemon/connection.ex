defmodule ProtohackersExs.SpeedDaemon.Connection do
  use GenServer, restart: :temporary

  alias ProtohackersExs.SpeedDaemon.{CentralTicketDispatcher, DispatchersRegistry, Message}
  ...
  defp parse_all_data(%__MODULE__{} = state) do
    case Message.decode(state.buffer) do
      ...
      :error ->
        send_message(state, %Message.Error{message: "illegal msg"})
        {:stop, :normal, state}
    end
  end
